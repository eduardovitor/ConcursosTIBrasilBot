...
CARGOS_PROFESSOR = ("Professor de informática efetivo",
                    "Professor de informática substituto")
...
def formatar_msg(dics_concursos):
    if isinstance(dics_concursos, str):
        ...
        dic_msg = []
        while (i < dic_tam):
            concurso = f"{dics_concursos[i]['concurso']}\n\n"
            link = f"{dics_concursos[i]['link']}\n\n"
            dic_msg.append(f"{concurso}{link}")
            i += 1
        msg = "".join(dic_msg)
    return msg
